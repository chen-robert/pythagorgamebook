...
{
  this.query("SELECT Books.title, Books.id, Books.author, Books.description, Books.dateCreated, Books.views, COUNT(Pages.id) AS pageCount FROM Pages INNER JOIN Books ON Pages.path LIKE CONCAT(Books.id, '%');", function(rows) {
      if (rows == null)
      {
